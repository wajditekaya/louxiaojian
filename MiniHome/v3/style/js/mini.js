...
(function(){
	  ...
	  var info_y=getPosition(Info).y;
	  function hoverMenu(id,hoverTag,xsTag){
		  if(!$(id)){return false}
		  if(!$(id).getElementsByTagName(this.hoverTag)){return false}
		  this.obj=$(id);
		  this.hoverTag=hoverTag;
		  this.xsTag=xsTag;
		  this.Ah=this.obj.getElementsByTagName(this.hoverTag);
		  this.cleanWhitespace=function(oEelement)
			  {
		  ...
		  for(var i=0;i<this.Ah.length;i++){
			  this.cleanWhitespace(this.Ah[i]);
			  if(this.Ah[i].getElementsByTagName(xsTag)[0]&&this.Ah[i].childNodes[0]){
				  this.Ah[i].onmouseover=function(){
					  if(this.className.indexOf('cur')==-1){this.className+=' cur';}
					  this.getElementsByTagName(xsTag)[0].style.display='block';
					  if((getPosition(this).y-info_y)>227){
						  if(this.getElementsByTagName(xsTag)[0].className.indexOf('tp')==-1){this.getElementsByTagName(xsTag)[0].className+=' tp'}
						  
						  };
				  }
				  this.Ah[i].onmouseout=function(){
					  if(this.className.indexOf('cur')!=-1){this.className=this.className.replace('cur','');}
					  this.getElementsByTagName(xsTag)[0].style.display='none';
					  if(this.getElementsByTagName(xsTag)[0].className.indexOf('tp')!=-1){this.getElementsByTagName(xsTag)[0].className=this.getElementsByTagName(xsTag)[0].className.replace('tp','')}
				  }
			  }
		  }
	  }
	  new hoverMenu("moveList2","li","ul");
	  new hoverMenu("moveList3","li","ul");
})();
(function(){
 if(!$('s')){return false}
 var s=$('s');
