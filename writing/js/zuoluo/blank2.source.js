...
ZBrowser.newListTab=function(S,addLast){
      ...
      /*scrollDistance=ullength-par_w;*/
	  Distance=ullength-par_w;
      
      function previous(){
			if(previousTime) clearTimeout(previousTime);
			if(nextTime) clearTimeout(nextTime);
			new function(){
				 if(parseInt(ul.style.left)<0){
					   ul.style.left=(parseInt(ul.style.left)+1)+'px';
					   previousTime=setTimeout(arguments.callee,10)
				 }else{
					 return false
				 }
			}
	  ...
	  function next(){
			...
			if(nextTime) clearTimeout(nextTime);
			new function(){
				 if(Math.abs(parseInt(ul.style.left))!=Distance){
					 ul.style.left=(parseInt(ul.style.left)-1)+'px';
					 nextTime=setTimeout(arguments.callee,10)
					}else{return false}
			}
	  }

	  addEvent(previousBut,'mouseover',previous);
	  addEvent(previousBut,'mouseout',function(){if(previousTime) clearTimeout(previousTime);});
	  addEvent(nextBut,'mouseover',next);
	  addEvent(nextBut,'mouseout',function(){if(nextTime) clearTimeout(nextTime);});
