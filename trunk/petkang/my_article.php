...
// $Id: my_article.php,v 1.9 2009/10/23 06:47:04 xltxlm Exp $
include "header.php";
...
class my_admin
{
	...
	function jigou_intro()
	{
		...
		mysql_query("update wenzhang_jigou set click=click+1  where j_id='{$_GET['j_id']}' ");
		$this->row= mysql_fetch_assoc(mysql_query("select * from wenzhang_jigou  where j_id='{$_GET['j_id']}' "));
		include "my_article/jigou_intro.html";
	}
	function shenfen()
	{
		include "db_locahost_mysql.php";
		...
		while ($row= mysql_fetch_assoc($stmt))
			$this->all_data[]= $row;
		include "my_article/shenfen.html";
	}
	...
	function shenfen_delete_do()
	{
		...
		header("location: my_article.php?act=shenfen");
	}
	...
	{
		include "db_locahost_mysql.php";
		$page= mysql_fetch_assoc(mysql_query("select count(*) c from  wenzhang_jigou"));
		$this->pageObj= new page($page['c']);
		$stmt= mysql_query("select * from  wenzhang_jigou   order by j_id desc  limit {$this->pageObj->limit_1},{$this->pageObj->limit_2}   ");
		$this->jigou_data= array ();
		while ($row= mysql_fetch_assoc($stmt))
			$this->jigou_data[]= $row;
		include "my_article/jigou_index.html";
	}
	function jigou_search_do()
	{
		...
		$page= mysql_fetch_assoc(mysql_query("select count(*) c from  wenzhang_jigou where instr(j_name,'{$_POST['j_name']}')>0 "));
		$this->pageObj= new page($page['c']);
		$stmt= mysql_query("select * from  wenzhang_jigou where instr(j_name,'{$_POST['j_name']}')>0 order by j_id desc  limit {$this->pageObj->limit_1},{$this->pageObj->limit_2}   ");
		$this->jigou_data= array ();
		while ($row= mysql_fetch_assoc($stmt))
			$this->jigou_data[]= $row;
		include "my_article/jigou_index.html";
	...
	function jigou_add_do()
	{
		...
		mysql_query("insert into wenzhang_jigou  " .
		...
		header("location: my_article.php?act=jigou_index");
	}
	function jigou_edit_do()
	{
		...
		mysql_query("update  wenzhang_jigou   set j_name='{$_POST['j_name']}',j_shenfen='{$_POST['j_shenfen']}',j_jibie='{$_POST['j_jibie']}', " .
	...
	function jigou_delete_do()
	{
		...
		if (!$_GET['j_id'])
			die;
		include "db_locahost_mysql.php";
		mysql_query("delete from wenzhang_jigou where j_id='{$_GET['j_id']}' limit 1 ");
		@ unlink("my_article_image/{$_GET['j_id']}.jpg");

		//更多的图片
		$stmt= mysql_query("select * from  wenzhang_jigou_image  where j_id='{$_GET['j_id']}' ");
		while ($row= mysql_fetch_assoc($stmt))
	...
	function jigou_edit()
	{
		...
		while ($row= mysql_fetch_assoc($stmt))
			$this->all_data[]= $row;
		$this->row= mysql_fetch_assoc(mysql_query("select * from wenzhang_jigou   where j_id='{$_GET['j_id']}' limit 1 "));
		include "my_article/jigou_edit.html";
	}
	function jigou_image_index()
	{
		...
		include "db_locahost_mysql.php";
		$this->row= mysql_fetch_assoc(mysql_query("select * from  wenzhang_jigou  where j_id='{$_GET['j_id']}' limit 1"));
		$stmt= mysql_query("select * from wenzhang_jigou_image  where j_id='{$_GET['j_id']}' order by i_id desc ");
		$this->all_image= array ();
		while ($row= mysql_fetch_assoc($stmt))
			$this->all_image[]= $row;
		include "my_article/jigou_image.html";
	}
	function jigou_image_add_do()
	{
		...
		if ($_FILES['file']['tmp_name'])
		{
			...
			mysql_query("insert into  wenzhang_jigou_image  " .
		...
		header("location: my_article.php?act=jigou_image_index&j_id={$_GET['j_id']}");
	}
	function jigou_image_delete_do()
	{
		...
		if (!$_GET['j_id'] || !$_GET['i_id'])
			die;
		include "db_locahost_mysql.php";
		$row= mysql_fetch_assoc(mysql_query("select *  from wenzhang_jigou_image where i_id='{$_GET['i_id']}'"));
		mysql_query("delete from wenzhang_jigou_image where i_id='{$_GET['i_id']}'");
	...
	{
		$count= mysql_fetch_assoc(mysql_query("select count(*) c from  wenzhang_jigou_image where j_id='{$j_id}' "));
		mysql_query("update wenzhang_jigou  set j_image_count='{$count['c']}' where j_id='{$j_id}' ");
	}
};